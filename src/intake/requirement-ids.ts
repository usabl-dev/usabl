 ...
 */
import { boundedOperatorPath, configError } from './config-error.js';
 ...
 * Each path is bounded before interpolation and interpolated as its own value, so a long first
 * path can never push the second location past the per-value cap and out of the message.
...
function duplicateReason(first: RequirementIdSite, repeat: RequirementIdSite): string {
  const repeatPath = boundedOperatorPath(repeat.path);
  const firstPath = boundedOperatorPath(first.path);
  const error =
