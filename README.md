...
Two of those rules can be steered. Only 401s from the origin of your `appBaseUrl` count, so a third-party service with its own stale credentials cannot block a scan. And a `reachedWhen` selector on a surface, once it matches, outranks the password-field rule, which is how a genuine change-password screen stays scannable. A same-origin 401 is never overridden.
