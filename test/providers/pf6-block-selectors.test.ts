...
import { LIVE_AND_PATH_INIT_SCRIPT, installPathHelper } from '../../src/deps/real.js';
...
/**
 * installPathHelper (the adoptPage path) must apply the same uniqueness check.
 * makeRealBrowserDriver uses LIVE_AND_PATH_INIT_SCRIPT at document start; adoptPage uses
 * installPathHelper after-the-fact. Both implementations must be kept in sync, so both
 * are tested independently here.
 */
describe('installPathHelper (adoptPage path) also rejects duplicate ids', () => {
  async function openWithInstallHelper(html: string): Promise<PwPage> {
    const page = await browser.newPage({ viewport: { width: 800, height: 900 } });
    await page.setContent(html, { waitUntil: 'domcontentloaded' });
    // installPathHelper is what adoptPage calls after a caller-navigated page is handed to usabl.
    // It must NOT be preceded by LIVE_AND_PATH_INIT_SCRIPT, or the guard returns early and the
    // TypeScript copy is never executed.
    await page.evaluate(installPathHelper);
    return page;
  }

  it('installPathHelper: duplicate ids produce distinct paths', async () => {
  ...
  it('installPathHelper: unique id still produces #id shortcut', async () => {
    const page = await openWithInstallHelper(`<!doctype html><html lang="en"><head><meta charset="utf-8" /></head><body>
  ...
  it('installPathHelper: pf-toast-live-region flags loose alert sharing an id with a contained alert', async () => {
    ...
    // pathForPage uses window.__usablPathFor, so it exercises the installPathHelper copy.
