/**
 * The whole chain, from a browser that landed on a sign-in page to the verdict and the pay-down
 * count, with the real check runner and the real rule in the path.
 *
 * The false claim this locks. On a real login-gated application whose scan session had expired,
 * every screen request answered with the application's login page. usabl scanned that page under
 * the requested screen's id, disclosed nothing, and reported all twenty-nine barriers on the
 * committed evidence floor as paid down. The verdict was red only because that login page happened
 * to carry barriers of its own. A clean login page would have produced verified, with a receipt,
 * and a claim that the accepted debt was gone. That is the false green this product exists to
 * prevent, so the pay-down claim is proved here rather than assumed from the gate's unit tests.
 *
 * The two runs below differ in exactly one fact: where the browser ended up. Everything else, the
 * config, the floor, the changed files, the provider, is identical. The signed-in run confirms the
 * barrier is paid down. The redirected run confirms nothing at all.
 */
...
function pageAt(landedUrl: string, hasPassword: boolean): Page {
  return makeFakePage({
    ...
    // case here would be unseen for that separate reason and would prove nothing about redirects.
    activePath: async () => 'html > body > main > button#save',
  });
}

/**
 * Fake git and filesystem, real check runner, fake browser standing at a chosen address. The rule
 * under test is the real one, so this run exercises the same code the CLI does.
 */
function depsLandingOn(landedUrl: string, hasPassword: boolean): Deps {
  const base = makeFakeDeps({
    files: POLICY_FILES,
    headContents: POLICY_FILES,
    writeTree: 'tree-redirect',
    changed: [{ code: 'M', path: 'fixtures/app/src/UsersPage.tsx' }],
  });
  return {
    ...base,
    checkRunner: makeCheckRunner({
      browser: { open: async () => pageAt(landedUrl, hasPassword), close: async () => {} },
...
describe('a screen the browser was redirected away from', () => {
  it('is a coverage gap that contributes no findings and pays down no floor entry', async () => {
    const r = await run(depsLandingOn('http://app.test/login?next=%2Fusers', true), config);

    // Disclosed, and the disclosure names where the browser went.
    ...
    // The login page's own barrier is not reported as a barrier on the users screen.
  ...
  it('never reports verified with paid down entries, whatever the login page contains', async () => {
    // The same run with a clean login page. Before this change that was the worst shape: no new
    // finding, no gap, so verified with a receipt and twenty-nine barriers claimed resolved.
    const base = depsLandingOn('http://app.test/login', true);
    const quiet: Deps = {
      ...base,
      checkRunner: makeCheckRunner({
    ...
    const r = await run(quiet, config);
  ...
  it('the same run with a working session does confirm the pay-down', async () => {
    // The control. Only the landing address changes. This is what proves the gap is what stopped
    // the pay-down claim, rather than some other property of the fixture.
    const r = await run(depsLandingOn(SCREEN_URL, false), config);

    expect(r.coverage.gaps.filter((gap) => gap.reason.includes('redirected away'))).toEqual([]);
  ...
  it('is scanned normally when a redirect lands on the screen that was asked for', async () => {
    ...
    const r = await run(depsLandingOn('http://app.test/users/', false), config);
