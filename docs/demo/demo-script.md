...
Only verified counts as proof. An idle run, where no covered surface changed, also lets the
assistant finish, and the film must not present idle as a pass.
...
4. It is honest by construction. On the surfaces it is configured for, it does not call a change
   ...
   them as clean. On a pull request the rules are read from the protected branch, so a change cannot
   rewrite the policy it is judged by. Locally, a changed policy file turns the verdict into
   approval required.
...
- Accessibility as a verdict with an exit code, not a list to triage. Four verdicts (verified,
  regression, not covered, approval required), where not-covered outranks verified, so a run with a
  known coverage gap is not called clean. A run that checked nothing, or that crashed, returns no
  verdict, and that is not a pass either.
- A re-checkable receipt on verified runs only. It records the exact source tree, policy, engine,
  scanner and browser versions. Re-verification recomputes those bindings and compares them; it does
...
- Reachability checks that a screen rendered more than an empty body, and, when a surface declares a
  `reachedWhen` selector, that the declared element was present. usabl proves it scanned the
  address it was configured or discovered to scan. It cannot prove that the address showed the
  screen a reader expects: a redirect, a signed-out state, or a feature flag can change what an
  address renders. A manual surface entry is an operator assertion, not evidence.
...
- Page text handed to a model sits inside a labeled frame that names it as data rather than
  instructions. The frame delimiters are removed from page text, so the page cannot close its own
  frame. Control sequences are stripped and credential-shaped values are redacted. usabl does not
  claim that a model will always treat the framed text as data; it claims what the frame does.
...
- One engine, six surfaces, one gate-owned verdict: command line, AI stop hook, browser overlay
  (advisory), pull request comment, CI required check, and the published docs pages. A Playwright
  helper reuses the flows a team already tests; it is a page-level check with no floor and no
  waivers, so its result can differ from the gated command.
...
- The overlay skips itself when the browser reports `navigator.webdriver` or the URL carries
  `?usabl=off`, so it stays out of a team's e2e tests and out of usabl's own scans.
...
`[END UNTRUSTED TEXT]`, and the frame delimiters are removed from page text so the page cannot
close its own frame. The gate has no model in its verdict path, so page text cannot talk it out
of a block. Proposed line: "And usabl hands the page's own text to the assistant sealed as data,
never as instructions, and the page cannot close that frame." Do not say the page "cannot"
influence the assistant; the frame is a control, not a guarantee about the model. This is a real
property and a novel intersection of AI, accessibility, and security, which is squarely on theme
for Innovation Days. It reads as strength, not cringe, when narrated plainly.
...
every waiver gets an owner and an expiry date), the customer payoff, and the bookend on the
